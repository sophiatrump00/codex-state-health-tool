...
if exist "%LocalAppData%\Python\bin\python.exe" (
    "%LocalAppData%\Python\bin\python.exe" "%SCRIPT%" provider-unpatch
    pause
    exit /b %ERRORLEVEL%
)
...
if not errorlevel 1 (
    python "%SCRIPT%" provider-unpatch
    pause
    exit /b %ERRORLEVEL%
)

if exist "%UserProfile%\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" (
    "%UserProfile%\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" "%SCRIPT%" provider-unpatch
    pause
    exit /b %ERRORLEVEL%
)
